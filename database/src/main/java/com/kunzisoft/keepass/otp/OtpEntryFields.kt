...
object OtpEntryFields {
     ...
     */
    fun parseFields(getField: (id: String) -> CharArray?): OtpElement? {
    ...
    fun parseOTPUri(otpUri: String): OtpElement? {
        val otpElement = OtpElement()
        if (parseOTPUri({ key -> if (key == OTP_FIELD) otpUri.toCharArray() else null }, otpElement))
            return otpElement
        return null
    }
     ...
     */
    private fun parseOTPUri(getField: (id: String) -> CharArray?, otpElement: OtpElement): Boolean {
        val otpPlainText = getField(OTP_FIELD)
        if (otpPlainText != null && otpPlainText.isNotEmpty() && isOTPUri(String(otpPlainText))) {
            val uri = Uri.parse(String(otpPlainText).removeSpaceChars())
    ...
    private fun parseTOTPFromOfficialField(getField: (id: String) -> CharArray?, otpElement: OtpElement): Boolean {
        ...
        try {
            ...
            otpElement.type = OtpType.TOTP
            if (lengthField != null) {
                otpElement.digits = String(lengthField).toIntOrNull() ?: OTP_DEFAULT_DIGITS
            }
            if (periodField != null) {
                otpElement.period = String(periodField).toIntOrNull() ?: TOTP_DEFAULT_PERIOD
            }
            if (algorithmField != null) {
                otpElement.algorithm =
                        when (String(algorithmField).uppercase(Locale.ENGLISH)) {
    ...
    private fun parseTOTPKeyValues(getField: (id: String) -> CharArray?, otpElement: OtpElement): Boolean {
        val plainText = getField(OTP_FIELD)
        if (plainText != null && plainText.isNotEmpty()) {
            val plainTextString = String(plainText)
            if (Pattern.matches(validKeyValueRegex, plainTextString)) {
                return try {
                    // KeeOtp string format
                    val query = breakDownKeyValuePairs(plainTextString)
                    otpElement.type = OtpType.TOTP
    ...
    private fun parseTOTPFromPluginField(getField: (id: String) -> CharArray?, otpElement: OtpElement): Boolean {
        ...
        try {
            ...
            if (settingsField != null) {
                ...
                val matcher = pattern.matcher(String(settingsField))
                if (!matcher.matches()) {
                    // malformed
                    return false
                }
    ...
    private fun parseHOTPFromOfficialField(getField: (id: String) -> CharArray?, otpElement: OtpElement): Boolean {
        ...
        try {
            ...
            if (secretCounterField != null) {
                otpElement.counter = String(secretCounterField).toLongOrNull() ?: HOTP_INITIAL_COUNTER
            }
        } catch (_: Exception) {
            return false
        }
    ...
    fun generateAutoFields(fieldsToParse: List<Field>): MutableList<Field> {
        ...
        newCustomFields.removeFirstWhen(OTP_FIELD)
        newCustomFields.removeFirstWhen(TOTP_SEED_FIELD)
        newCustomFields.removeFirstWhen(TOTP_SETTING_FIELD)
        newCustomFields.removeFirstWhen(HMACOTP_SECRET_FIELD)
        newCustomFields.removeFirstWhen(HMACOTP_SECRET_HEX_FIELD)
        newCustomFields.removeFirstWhen(HMACOTP_SECRET_BASE32_FIELD)
        newCustomFields.removeFirstWhen(HMACOTP_SECRET_BASE64_FIELD)
        newCustomFields.removeFirstWhen(HMACOTP_SECRET_COUNTER_FIELD)
        newCustomFields.removeFirstWhen(TIMEOTP_SECRET_FIELD)
        newCustomFields.removeFirstWhen(TIMEOTP_SECRET_HEX_FIELD)
        newCustomFields.removeFirstWhen(TIMEOTP_SECRET_BASE32_FIELD)
        newCustomFields.removeFirstWhen(TIMEOTP_SECRET_BASE64_FIELD)
        newCustomFields.removeFirstWhen(TIMEOTP_LENGTH_FIELD)
        newCustomFields.removeFirstWhen(TIMEOTP_PERIOD_FIELD)
        newCustomFields.removeFirstWhen(TIMEOTP_ALGORITHM_FIELD)
        // Empty auto generated OTP Token field
        if (fieldsToParse.containsWhen(OTP_FIELD)
                || fieldsToParse.containsWhen(TOTP_SEED_FIELD)
                || fieldsToParse.containsWhen(HMACOTP_SECRET_FIELD)
                || fieldsToParse.containsWhen(HMACOTP_SECRET_HEX_FIELD)
                || fieldsToParse.containsWhen(HMACOTP_SECRET_BASE32_FIELD)
                || fieldsToParse.containsWhen(HMACOTP_SECRET_BASE64_FIELD)
                || fieldsToParse.containsWhen(TIMEOTP_SECRET_FIELD)
                || fieldsToParse.containsWhen(TIMEOTP_SECRET_HEX_FIELD)
                || fieldsToParse.containsWhen(TIMEOTP_SECRET_BASE32_FIELD)
                || fieldsToParse.containsWhen(TIMEOTP_SECRET_BASE64_FIELD)
        )
